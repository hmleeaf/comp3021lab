...
public class Heap<T extends Comparable<T>> {
    ...
    private void heapifyUp() {
        // TODO
        int targetIndex = container.size() - 1;
        while (hasParent(targetIndex)
                && (container.get(targetIndex).compareTo(container.get(getParentIndex(targetIndex))) < 0)) {
            swap(targetIndex, getParentIndex(targetIndex));
            targetIndex = getParentIndex(targetIndex);
        }
    }
